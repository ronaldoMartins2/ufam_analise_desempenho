...
def intervalo_confianca_sts_csum(list, bloco_von_local):
    list_len = len( list )
    X_ = media_amostral( list )

    sum = 0
    denominador = (bloco_von_local*bloco_von_local*bloco_von_local) - bloco_von_local
    
    for ele in list:
        somador = ( bloco_von_local * ( X_ - ele) ) * ( bloco_von_local * ( X_ - ele) )
        sum += ( ( (12 * ele) * (12 * ele) ) / denominador ) +  somador

    DOIS_B = (2*bloco_von_local)-1
    HCSUM =  stats.t.ppf(0.05, DOIS_B) * math.sqrt( sum / list_len * ( DOIS_B ) )

    return [X_ - HCSUM, X_ + HCSUM]
...
def sts_calc(list_block, bloco_von_local):
    sum = 0
    count = 1
    for ele in list_block:
        sum += ( ( (bloco_von_local + 1) / 2) - count) * ele
        count += 1
    return sum

def medias_blocos_sts_area(lista_pos_transiente, bloco_von_local):
    #global bloco_von
    global SBM_S
    ...
    for ele in lista_pos_transiente:
        ...
        if(count % bloco_von_local == 0):
            ...
            #elementos_bloco_von_less_S = elementos_bloco_von_copy[:-SBM_S]
            elementos_bloco_von_less_S = elementos_bloco_von_copy

            media_bloco_von = sts_calc( elementos_bloco_von_less_S, bloco_von_local )
...
def von_neuman(lista_pos_transiente):
    global bloco_von
    global SBM_S
...
def teste_shapiro(list_bloco):
    global z_95
    '''
    list_bloco.sort()

    X_ = media_amostral( list_bloco )

    SS = 0
    for ele in list_bloco:
        SS += (ele - X_) * (ele - X_)

    len_list = len(list_bloco)

    metade_list = []
    metade_list = list_bloco.copy()
    if ( ( len_list % 2) == 0):
        metade_list = list1[:len_list/2]
    else:
        metade_list = list1[:( len_list // 2 )+1]

    sum_b = 0
    count_n = 0
    for ele_n in metade_list:
        sum_b +=  () * (ele_n[len_list-count_n+1] - ele_n)
        count_n += 1


    w_calc = (sum_b * sum_b) / SS
    '''
    
    p_value = stats.shapiro(list_bloco)

    return p_value.statistic < z_95
...
class server(Thread):
    ...
    def run(self):
        ...
        while ( remover_transiente  | (not normalidade) | continuar_simulacao ) :
                    ...
                    if not remover_transiente:
                        ...
                        if( (B_Von * bloco_von) ==  count_pos_transiente):
                            ...
                            if(not normalidade):
                                ...
                                sts_values_list = medias_blocos_sts_area(tempo_espera_pos_transiente, bloco_von)

                                normalidade = teste_shapiro( sts_values_list )
                                ...
                                #nao_normalidade = not (von_value < beta_von)
                                bloco_von += 50
                            else:

                                medias_blocos_calc = medias_blocos_sts_area( tempo_espera_pos_transiente, bloco_von)

                                X_average = media_amostral( medias_blocos_calc )
                                interval_confianca = intervalo_confianca_sts_csum( medias_blocos_calc, bloco_von)
