...
def intervalo_confianca_sts(list, bloco_von_local):
    list_len = len( list )
    X_ = media_amostral( list )

    sum = 0
    for ele in list:
        sum += (ele*ele) / (list_len*list_len) * ( (bloco_von_local*bloco_von_local) -1)

    HA =  stats.t.ppf(0.05, list_len) * math.sqrt( 12 * sum )
                    ...
                    if not remover_transiente:
                        ...
                        if( (B_Von * bloco_von) ==  count_pos_transiente):
                            ...
                            if(not normalidade):
                                ...
                                interval_confianca = intervalo_confianca_sts( medias_blocos_calc, bloco_von)
